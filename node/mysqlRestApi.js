...
app.all('/regist', function(req, res) {
	...
	if(getdata.username != '' && getdata.password != '' && getdata.username != undefined && getdata.password != undefined) {
		console.log(JSON.stringify(getdata));
		// var addSql = 'INSERT INTO websites(Id,name,url,alexa,country) VALUES(0,?,?,?,?)'; //mysql使用新增等方法
		// var addSqlParams = [getdata.username, 'https://c.runoob.com', getdata.password, 'CN']; //mysql使用新增的方法
		// console.log(addSql);
		//从数据库写入如数
		connection.collection("vipsysterm").insertOne(getdata, function(error, results) {
			console.log('插入成功')
			if(error) {
				console.log('[INSERT ERROR] - ', error.message);
